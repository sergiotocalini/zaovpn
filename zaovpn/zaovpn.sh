...
usage() {
    ...
    echo "Examples:"
    echo ""
    echo "  ~# ${APP_NAME} -s certs -j ID:STATUS"
    echo "  {" 
    echo "     \"data\":["
    echo "         { \"{#ID}\":\"sergio.tocalini.sha256.2048\", \"{#STATUS}\":\"0\" }"
    echo "      ]"
    echo "  }"
    echo "  ~# ${APP_NAME} -s cert -a p=\"sergio.tocalini.sha256.2048\" -a p=after"
    echo "  1580550683"
    echo "  ~# ${APP_NAME} -s cert -a p=\"sergio.tocalini.sha256.2048\" -a p=expires"
    echo "  317"
    echo "  ~#"
    echo ""
